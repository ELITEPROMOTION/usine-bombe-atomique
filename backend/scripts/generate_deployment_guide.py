#!/usr/bin/env python3
"""Generate the personalised step-by-step deployment guide for Ahmed.

Reads `deploy/config/ahmed_answers.json` (produced by the wizard's `init`
phase) and writes `DEPLOYMENT_AHMED_STEP_BY_STEP.md` at the repo root.

Run:
    python backend/scripts/generate_deployment_guide.py
    python backend/scripts/generate_deployment_guide.py --answers path.json --output OUT.md
"""
...
DEFAULT_ANSWERS = REPO_ROOT / "deploy" / "config" / "ahmed_answers.json"
DEFAULT_OUTPUT = REPO_ROOT / "DEPLOYMENT_AHMED_STEP_BY_STEP.md"


def load_answers(path: Path) -> dict[str, Any]:
    if not path.exists():
        # Fall back to a documented placeholder so the guide still generates
        # for review purposes (the wizard will overwrite this file).
        return {
...
def render(answers: dict[str, Any]) -> str:
    ...
    email = answers["ahmed_email"]
    domain = answers["ahmed_domain"]
    phone = answers["ahmed_phone_e164"]
    region = answers["hetzner_region"]
    plan = answers["hetzner_plan"]
    tz = answers["timezone"]
    smtp = answers["smtp_provider"]
    backups = answers.get("auto_backup_enabled", True)

    return dedent(f"""\
    # Deploiement UBA en production — Guide pas a pas pour Ahmed

    > Genere automatiquement le **{today}** depuis tes reponses du wizard.
    > Compte cible: **{email}** · Domaine: **{domain}** · Phone: **{phone}**
    > VPS Hetzner: plan **{plan}** en region **{region}** · Timezone: **{tz}**
    > Auto-backup: **{"OUI" if backups else "NON"}** · SMTP: **{smtp}**

    Ce guide t'accompagne *de zero a UBA en ligne sur https://{domain}*.
    ...
    1. Utilise **{email}** comme adresse principale (sauf indication).
    ...
    - Plan recommande: **{plan}** (4 vCPU AMD / 8 GB RAM / 160 GB NVMe — env. 14 EUR/mois)
    - Region a choisir: **{region}** (proche de l'audience cible)
    - Capture d'ecran attendue: **Console > Projects** avec un projet vide

    ### 1.2 Cloudflare (DNS + TLS proxy) — *obligatoire*
    - URL: https://dash.cloudflare.com/sign-up
    - Apres inscription: **Add a site** → entre **{domain}**
    - Cloudflare te donnera 2 nameservers (`xxx.ns.cloudflare.com`); va chez ton
      registrar de domaine pour les configurer.
    - Capture attendue: zone **{domain}** en statut "Active" (peut prendre 24h).

    ### 1.3 Scaleway (object storage backups) — {"obligatoire" if backups else "*OPTIONNEL*"}
    {"Obligatoire car auto-backup est ACTIVE." if backups else "Tu as desactive l'auto-backup, tu peux sauter cette etape."}
    - URL: https://console.scaleway.com/register
    - Apres inscription: **Project Dashboard > IAM > API keys > Generate**

    ### 1.4 Registrar du domaine — *obligatoire si tu n'as pas {domain}*
    - Si tu possedes deja **{domain}**, saute cette etape.
    ...
    - Genere une cle SSH ED25519 sur ta machine:
      ```bash
      ssh-keygen -t ed25519 -C "{email}" -f ~/.ssh/uba_deploy_ed25519
      ```
    ...
    - Token: **My Profile > API Tokens > Create Token**
      - Template: "Edit zone DNS" → Zone Resources: include zone **{domain}**
    ...
    ```
    5 verifications:
    - `https_reachable` — https://{domain} repond
    ...
    | Ressource         | URL                                            |
    |-------------------|------------------------------------------------|
    | App principale    | https://{domain}                               |
    | Dashboard CEO     | https://{domain}/ceo                           |
    | Ahmed Inbox       | https://{domain}/ahmed_inbox                   |
    | API documentation | https://{domain}/docs                          |
    | Observability     | https://{domain}/observability                 |
    | Domaines          | https://{domain}/domains                       |
    ...
    Ouvre https://{domain}/observability → onglet **Metrics** ou **Errors**.
    ...
    ```bash
    for i in {{1..50}}; do curl -s https://{domain}/api/v1/health >/dev/null; done
    ```
    ...
    - [ ] Active la **2FA TOTP** sur le compte admin **{email}**.
    ...
    ### 8.3 SSL/TLS
    - [ ] Test SSL Labs: https://www.ssllabs.com/ssltest/analyze.html?d={domain}
      → cible: **A+**.
    - [ ] Verifie HSTS: `curl -sI https://{domain} | grep -i strict-transport`.
    ...
    - `https://{domain}/observability?tab=overview`
    - `https://{domain}/observability?tab=metrics`
    - ...
    ...
    - Passe de **{plan}** a `cpx41` (8 vCPU / 16 GB) → +20 EUR/mois.
    ...
    **Q: Combien ca coute par mois?**
    R: Hetzner ({plan}) ~14 EUR + Cloudflare 0 EUR + Scaleway storage ~3 EUR + Claude API selon usage = **environ 20 EUR/mois fixe** + variable Claude.
    ...
    **Q: La verification d'identite Hetzner echoue?**
    ...
    - **Domaine deploye**: {domain}
    - **Compte principal**: {email}
    - **Telephone d'urgence**: {phone}
    - **Region VPS**: {region}
    - **Plan VPS**: {plan}
    - **Timezone**: {tz}
    ...
    *Genere le {today} par `backend/scripts/generate_deployment_guide.py`. Re-execute le script si tes reponses changent.*
    """)


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--answers", default=str(DEFAULT_ANSWERS),
                     help="Path to ahmed_answers.json")
    ...
    print(f"Wrote {out} ({line_count} lines, "
            f"personnalise pour {answers['ahmed_domain']})")
    return 0
